@extends('layout')
@section('title', "{$book->title} -{$page->number} - $text")
@section('content')
    <form class="container min-h-screen p-2 space-y-2" method="post" action="{{ route('phrases.store') }}">
        @csrf
        <input type="hidden" name="page_id" value="{{ $page->id }}">
        <textarea hidden name="highlights">{!! $highlights !!}</textarea>
        <div class="flex flex-col justify-items-stretch gap-2">
            <div class="form-control">
                ...
                <textarea name="text" id="text" class="textarea" rows="1">{{ $text }}</textarea>
            </div>
            <div class="form-control">
                <label class="label-text" for="meaning">Meaning Persian:</label>
                <textarea name="meaning[fa]" id="meaning-fa" class="textarea" rows="1">{{ $translated['fa'] }}</textarea>
            </div>
            <div class="form-control">
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            (function ($) {
                translateText()
                @foreach($languages as $lang => $isOn)
                @if($isOn)
                openTranslateText('google', '{{ $lang }}')
                @endif
                @endforeach
            })(window.jQuery)
        })
